...
namespace DevIO.Business.Models.Validations.Docs
{
    public class CpfValidation
    {
        ...
        private static bool TemDigitosValidos(string valor)
        {
            ...
            return string.Concat(primeiroDigito, segundoDigito) == valor.Substring(TamanhoCpf - 2, 2);
        }
    }
    ...
    public class CnpjValidation
    {
        ...
        private static bool TemDigitosValidos(string valor)
        {
            ...
            var digitoVerificador = new DigitoVerificador(number)
                .ComMultuplicadoresDeAte(2, 9)
                .Substituindo("0", 10, 11);
            var primeiroDigito = digitoVerificador.CalcularDigito();
            digitoVerificador.AddDigito(primeiroDigito);
            var segundoDigito = digitoVerificador.CalcularDigito();

            return string.Concat(primeiroDigito, segundoDigito) == valor.Substring(TamanhoCnpj - 2, 2);
        }
    ...
    public class DigitoVerificador
        {
            ...
            private readonly IDictionary<int, string> _subtituindo = new Dictionary<int, string>();
            private bool _complementarModulo = true;
            ...
            public DigitoVerificador ComMultuplicadoresDeAte(int primeiroDigito, int ultimoDigito)
            {
                _multiplicadores.Clear();
                for(var i = primeiroDigito; i <= ultimoDigito; i++)
            ...
            private string GetDigitSum()
            {
                ...
                var mod = (soma % Modulo);
                var resultado = _complementarModulo ? Modulo - mod : mod;

                return _subtituindo.ContainsKey(resultado) ? _subtituindo[resultado] : resultado.ToString();
            }
